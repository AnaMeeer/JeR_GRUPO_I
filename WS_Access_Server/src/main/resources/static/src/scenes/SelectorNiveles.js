export default class SelectorNiveles extends Phaser.Scene {
    ...
    create(data) {
        ...
        this.tutorial = this.add.image(this.sys.game.config.width / 2, this.sys.game.config.height / 2, 'Tutorial').setInteractive({ useHandCursor: true }).setScale(0.6)
            ...
            //ELIMINAR CUANDO SE TERMINEN DE HACER CAMBIOS
        this.BORRAR = this.add.image(this.sys.game.config.width / 2 + 100, this.sys.game.config.height / 2 + 50, 'NivelInfinito').setInteractive({ useHandCursor: true }).setScale(0.6)
            .on("pointerover", () => {
                this.BORRAR.setScale(0.65);

            })
            .on("pointerout", () => {
                this.BORRAR.setScale(0.6);
            })
            .on("pointerdown", () => {
                this.musica.stop();
                this.scene.start("EscenaSonido", { musica: that.musica, musicaInGame: that.musicaInGame, click1: that.click1Sound, click2: that.click2Sound, sonidos: that.sonidos });
                this.scene.start("EscenaPausa", { musica: that.musica, musicaInGame: that.musicaInGame, click1: that.click1Sound, click2: that.click2Sound, sonidos: that.sonidos, escena: 7 });
                this.scene.start('scene_PlayBORRAR', { escena: this, player: data.player });
            });
